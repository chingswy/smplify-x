...
def create_dataset(dataset='openpose', data_folder='data', **kwargs):
    ...
    elif dataset.lower() == 'openpose_new':
        return OpenPoseKp(data_folder, **kwargs)
    else:
...
def read_keypoints_new(info, idx, use_hands=True, use_face=True,
                   use_face_contour=False):
    keypoints = []
    ...
    gender_gt = []
    while True:
        body_keypoints = info['keypoints2d'][idx]
        if use_hands:
            left_hand_keyp = info['handl2d'][idx]
            right_hand_keyp = info['handr2d'][idx]
            body_keypoints = np.concatenate(
                [body_keypoints, left_hand_keyp, right_hand_keyp], axis=0)
        if use_face:
            # TODO: Make parameters, 17 is the offset for the eye brows,
            # etc. 51 is the total number of FLAME compatible landmarks
            face_keypoints = info['face2d'][idx, 17: 17 + 51, :]
...
class OpenPoseKp(Dataset):

    NUM_BODY_JOINTS = 25
    NUM_HAND_JOINTS = 20

    def __init__(self, data_folder, info=None,
                 use_hands=False,
                 use_face=False,
                 dtype=torch.float32,
                 model_type='smplx',
                 ...
                 openpose_format='coco25',
                 **kwargs):
        super(OpenPoseKp, self).__init__()
    ...
    def next(self):
        ...
        return self.read_item(img_path, self.cnt)